// 인원 연령대 분석 + 예산 계산 + 리뷰 기반 메뉴/사유/맵기/아이친화도 추출 + 거리 필터 + 이동시간 반영 일정표 생성
...
// 시속 32km 가정(신호대기, 골목길 포함한 대략치)으로 두 지점 사이 이동 시간(분) 추정
...
function scorePlace(place, ageProfile, tier) {
  let score = 0;
  if (place.rating) score += (place.rating / 5) * 28;
  if (place.ratingCount) score += Math.min(place.ratingCount / 500, 1) * 10;
  if (place.crossVerified) score += 14;
  ...
  if (placeLevel <= tier.maxPriceLevel) score += 20;
  else score += Math.max(0, 20 - (placeLevel - tier.maxPriceLevel) * 12);
...
/** 이동시간을 반영해서 시각을 계산하며 슬롯을 하나씩 쌓아가는 헬퍼 */
...
function buildDayItinerary(days, restaurants, cafes, attractions, lodging, restStops, flightInfo = {}) {
  ...
  for (let day = 1; day <= days; day++) {
    ...
    if (isFirst) {
      const startMinutes = parseClockToMinutes(flightInfo.departureTime) ?? 540; // 09:00 기본 출발
      ...
      const cursor = { minutes: 480, coord: lodgingCoord }; // 08:00, 숙소에서 시작
      const push = makeSlotPusher(slots, cursor);
