// 인원 연령대 분석 + 예산 티어 계산 + 장소 스코어링 + 여유로운 시간대별 동선 생성
...
/** 여유로운 하루 일정: 오전 관광 1곳 + 점심 + 오후 관광 1곳 + 카페 1곳 + 저녁 (+ 숙소) */
function buildDayItinerary(days, restaurants, cafes, attractions, lodgings) {
  ...
  for (let day = 1; day <= days; day++) {
    ...
    if (day > 1 && lodging) {
      stops.push({
        time: '아침', name: lodging.name, address: lodging.address, category: '숙소',
        typeLabel: '숙소 조식', reason: '전날 묵은 숙소에서 여유롭게 아침 식사',
      });
    }

    const morningAttraction = toStop(pick(attractions, attrIdx++), '관광');
    if (morningAttraction) stops.push({ time: '오전 (10:30)', ...morningAttraction });

    const lunch = toStop(pick(restaurants, restIdx++), '식사');
    if (lunch) stops.push({ time: '점심 (12:30)', ...lunch, typeLabel: `점심 · ${lunch.typeLabel || '식사'}` });

    const afternoonAttraction = toStop(pick(attractions, attrIdx++), '관광');
    if (afternoonAttraction) stops.push({ time: '오후 (14:30)', ...afternoonAttraction });

    const cafeStop = toStop(pick(cafes, cafeIdx++), '카페');
    if (cafeStop) stops.push({ time: '카페 (16:00)', ...cafeStop });

    const dinner = toStop(pick(restaurants, restIdx++), '식사');
    if (dinner) stops.push({ time: '저녁 (18:00)', ...dinner, typeLabel: `저녁 · ${dinner.typeLabel || '식사'}` });

    if (lodging) {
      if (day < days) {
        stops.push({
          time: '숙박', name: lodging.name, address: lodging.address, category: '숙소',
          typeLabel: '숙소 체크인', reason: lodging.reason, priceEstimate: lodging.priceEstimate,
        });
      } else {
