// 인원 연령대 분석 + 예산 계산(가격대 비례 가산점) + 리뷰 기반 메뉴/사유/맵기/아이친화도 추출 + 거리 필터 + 이동시간 반영 일정표 생성 + 네이버 단독 맛집 보강
...
/** 구글 후보 목록엔 없고 네이버에만 있는 곳들을, 네이버 순위(리뷰 많은 순) 그대로 상위 N개만 뽑아요 */
...
function buildDayItinerary(days, restaurants, cafes, attractions, lodging, restStops, flightInfo = {}) {
  ...
  for (let day = 1; day <= days; day++) {
    ...
    if (isFirst) {
      ...
      const cafeOpts = pickCafe(1).map((p) => toOption(p, '카페'));
