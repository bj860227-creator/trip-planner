...
/** 검색 지역명이 실제 주소에 포함된 곳만 남겨서, 엉뚱한 지역 결과를 걸러내요 */
function filterByLocation(places, locationKeyword) {
  ...
  return filtered.length ? filtered : places; // 다 걸러지면(너무 빡빡하면) 원래 목록 유지
}

/** 숙소가 리조트/콘도/펜션류인지 판단 — 부대시설 이용 옵션을 추가하기 위함 */
...
/**
 * 시간대별 일정표 생성
 * - 첫날: 도착 → 점심 → 체크인(15시) → 체크인 후 자유시간 → 카페 → 저녁
 * - 마지막날: 조식 → 오전 관광 → 체크아웃(11시) → 점심 → (귀국 항공편 있으면 종료)
 * - 중간날: 조식 → 오전 관광 → 점심 → 오후 관광 → 카페 → 저녁
 * - 리조트/콘도/펜션이면 관광 슬롯에 "숙소 내 부대시설" 옵션도 같이 제공
 */
...
module.exports = {
  ...
  buildDayItinerary, rankPlaces, filterOutChains, filterByLocation,
};
