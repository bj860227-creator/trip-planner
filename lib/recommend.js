...
// 이름/리뷰에서 "매운맛 위주" 신호를 감지해요 (완전 제외는 아니고 감점만)
...
function scorePlace(place, ageProfile, tier) {
  ...
  if (place.isInHouse) score += 20; // 숙소 안에 있는 시설이면 이동 없이 갈 수 있어서 우선
  ...
  if (ageProfile.hasYoungChildren) {
    ...
    if (place.isSpicy) score -= 22; // 매운 위주 식당이면 어린 자녀 동반 시 감점
...
function buildDayItinerary(days, restaurants, cafes, attractionsMorning, attractionsAfternoon, lodging, flightInfo = {}) {
  ...
  for (let day = 1; day <= days; day++) {
    ...
    if (isFirst) {
      ...
      const cafeOpts = pickCafe(2).map((p) => toOption(p, '카페'));
      if (cafeOpts.length) slots.push({ time: '10:30', label: '카페', type: 'cafe', options: cafeOpts });

      if (lodging) {
        slots.push({ time: '11:30', label: '숙소 체크아웃', type: 'lodging',
