// 인원 연령대 분석 + 예산 계산 + 리뷰 기반 메뉴/사유/맵기/아이친화도 추출 + 거리 필터 + 시간대별 일정표 생성
...
// 기준값을 현실적인 금액대로 잡아서, 어지간한 가족여행 예산으로는 천장에 덜 부딪히게 했어요
...
function buildDayItinerary(days, restaurants, cafes, attractions, lodging, restStops, flightInfo = {}) {
  ...
  for (let day = 1; day <= days; day++) {
    ...
    if (isFirst) {
      ...
      if (restStops.length) {
        slots.push({ time: '가는 길', label: '휴게소 (식사 & 커피)', type: 'meal',
          options: [toOption(restStops[0], '휴게소')] });
      }
      if (lodging) {
      ...
      if (attrOpts.length) slots.push({ time: '오후', label: '근처 관광지', type: 'attraction', options: attrOpts });

      const dinnerOpts = pickRest(2).map((p) => toOption(p, '식당'));
      if (dinnerOpts.length) slots.push({ time: '저녁', label: '저녁 (관광지 근처)', type: 'meal', options: dinnerOpts });

    } else if (isLast) {
      if (lodging) {
        slots.push({ time: '08:00', label: '조식', type: 'breakfast',
          options: [{ name: lodging.name, address: lodging.address, menu: '숙소 조식', reason: '짐 정리와 함께 여유롭게 아침 식사', priceEstimate: null }] });
      }
      if (lodging) {
      ...
      const attrOpts = pickAttr(1).map((p) => toOption(p, '관광'));
      if (attrOpts.length) slots.push({ time: '오전~낮', label: '관광지', type: 'attraction', options: attrOpts });

      const lunchOpts = pickRest(2).map((p) => toOption(p, '식당'));
      if (lunchOpts.length) slots.push({ time: '점심', label: '점심', type: 'meal', options: lunchOpts });

      const cafeOpts = pickCafe(1).map((p) => toOption(p, '카페'));
      if (cafeOpts.length) slots.push({ time: '커피', label: '카페', type: 'cafe', options: cafeOpts });

      if (restStops.length) {
        const rs = restStops[1] || restStops[0];
        slots.push({ time: '가는 길', label: '집 가는 길 휴게소', type: 'meal', options: [toOption(rs, '휴게소')] });
      }
      ...
      if (lodging) {
        const breakfastOpts = [{ name: lodging.name, address: lodging.address, menu: '숙소 조식', reason: '묵고 계신 숙소에서 아침 식사', priceEstimate: null }];
        slots.push({ time: '08:00', label: '조식', type: 'breakfast', options: breakfastOpts });
      }
      const attrOpts = pickAttr(1).map((p) => toOption(p, '관광'));
      if (resort && lodging) attrOpts.push(resortOption(lodging));
      if (attrOpts.length) slots.push({ time: '이동 · 관광', label: '관광지', type: 'attraction', options: attrOpts });

      const lunchOpts = pickRest(2).map((p) => toOption(p, '식당'));
      if (lunchOpts.length) slots.push({ time: '점심', label: '점심 (관광지 근처)', type: 'meal', options: lunchOpts });

      const cafeOpts = pickCafe(2).map((p) => toOption(p, '카페'));
      if (cafeOpts.length) slots.push({ time: '카페', label: '카페', type: 'cafe', options: cafeOpts });
